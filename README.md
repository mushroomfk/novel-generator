...
- Agent 有轨迹：讨论、资料分析、章节写作、队列化审校和整书架构共用执行时间线；执行状态、动作契约和子任务记录会写入项目目录；技能库可查看自学习候选、确认草案、写作回归、模型审查、失败案例、技能版本、技能包和能力看板
...
| Agent 执行 | 讨论、资料分析、受监督逐章生产、并行候选审校、可配置低分自动修订、整书架构、workflow 状态、动作契约、执行轨迹、经验候选、自学习复盘、失败案例库 |
...
- 执行链路：Agent 计划、执行、子任务进度、批量章节队列、结果和错误状态通过 backend 统一管理，前端以流式状态展示；每次执行会在 `.gaoxia/runs/{task_id}/workflow.json` 记录预检、动作契约、心跳、产物校验和子任务文件；任务结束后记录技能使用、调用规则候选、写作评价和失败案例，技能库提供自学习查看、候选确认、写作回归、模型审查、排程配置、后台排程、技能版本回滚、技能包迁移和全局技能提升入口
