...
- Agent 有轨迹：讨论、资料分析、章节写作、队列化审校和整书架构共用执行时间线；主对话可直接提交长文本，后端保存完整输入，超过 20000 字符的用户输入会先筛掉明显无关、技术日志或网页样板段落，再按约 50000 字符分段导入项目资料库，并把资料标题加入本轮引用；给路由、规划、讨论和技能整理模型看的上下文会自动压缩为摘要、原文头尾片段和资料引用，避免输入过大导致请求失败；执行消息会把 `event_blocks` 按计划阶段、执行阶段和结果阶段展示；执行状态、动作契约和子任务记录会写入项目目录；前端可按 `task_id` 读取 workflow 状态，停止长任务时会向后端写入中断请求，Agent 会在动作边界停止后续动作；内置技能目录记录对应的 Agent action、mode 和确认要求，规划器看到的技能清单与执行 action 保持一致，主对话快捷按钮也会把对应内置技能 ID 传给后端，让“续写本章”“判断本章”等入口按同一套 action 元数据生成计划；技能库可查看自学习候选、确认草案、写作回归、去 AI A/B、真实章节项目样本池、去 AI 裁判、去 AI 智能巡检、项目去 AI 自学习规则、模型审查、失败案例、系统学习版文风 / XP、剧情债务与人物弧线、技能版本、技能包和能力看板
...
| 资料库 | 导入 `txt / md / json / csv / html / docx / pdf`，PDF 可用 LiteParse 做本地增强解析并保留页码标记，建立本地索引；可选接入 Obsidian Vault |
...
| Agent 执行 | 讨论、资料分析、受监督逐章生产、并行候选审校、可配置核验自动修订、整书架构、主对话快捷技能调度、workflow 状态、动作契约、执行轨迹、经验候选、自学习复盘、失败案例库 |
...
- 模型接入：使用 OpenAI-compatible `chat/completions`，可接入 OpenAI、DashScope、火山方舟等兼容服务；传输层会移除部分兼容服务容易拒收的可选采样参数，并对 SSL EOF、远端中途断开、临时连接错误和可重试 5xx / 429 最多重试 3 次
...
- Obsidian 状态同步：自学习面板保存 Obsidian 配置或手动同步 Vault 时，接口会随作品详情返回最新自学习状态，让新增 Vault 笔记触发的图谱风险、维护摘要和章节任务卡立即出现在面板里；保存草稿、发布到 Vault、确认 Vault 合并、忽略或恢复维护建议后，维护接口也会返回最新自学习状态，前端优先用响应里的状态更新维护摘要和章节任务卡，并重新读取作品详情；发布和确认合并还会刷新 Obsidian 同步状态。维护动作已成功但自学习状态或作品详情刷新失败时，界面会保留成功提示并附带失败原因。
