...
def _discover_character_names(texts: list[str], limit: int = 24) -> list[str]:
  ...
  for text_index, text in enumerate(texts):
    ...
    for pattern in (_COMPOUND_SURNAME_PATTERN, _SINGLE_SURNAME_PATTERN):
      for match in pattern.finditer(source):
        ...
        if _candidate_character_context_has_person_evidence(source, match.start(1), candidate):
          evidence_counts[candidate] = evidence_counts.get(candidate, 0) + 1
  ...
  filtered = [
    ...
    if evidence_counts.get(name, 0) >= 1 and count >= (2 if len(name) >= 3 else 3)
  ]
  filtered.sort(
    key=lambda name: (
      -counts[name],
