...
- 修改摘要：版本号提升到 `0.1.3`，同步 `package.json`、`package-lock.json`、Tauri 配置、Cargo 配置、README 状态说明、Windows 打包说明和 macOS 测试版安装说明；基于当前工作区生成 macOS arm64 测试包，并通过 GitHub Actions `Windows Desktop Release` 生成 Windows x64 测试包。
- 影响范围：桌面测试包版本、macOS / Windows 测试说明、Tauri release 产物和当前分支打包流程；不改变模型配置结构、项目数据格式、许可证格式或运行时 API。
- 验证结果：2026-06-10 执行 `npm run release:test:macos` 时，`npm run verify:desktop` 已通过打包静态检查、431 个后端 unittest、前端生产构建和 Python sidecar 打包，但在独立 sidecar 健康检查阶段顶层命令以 `137` 退出且没有错误文本，不能作为完整通过证据；随后手动复验同一个 `0.1.3` sidecar，`GET /api/app/health`、`POST /api/config/test` 本地 Embedding 和 `POST /api/app/shutdown` 均通过，Embedding 返回 `BAAI/bge-small-zh-v1.5 可用，向量维度 512`，sidecar 退出码为 `143`。继续手动执行 Tauri release 构建并生成 `src-tauri/target/release/bundle/dmg/稿匣_0.1.3_aarch64.dmg`；`bash scripts/repair-tauri-app-signature.sh '稿匣' release` 和 `codesign --verify --deep --strict --verbose=2 src-tauri/target/release/bundle/macos/稿匣.app` 通过；`.app` 主程序和内置 sidecar 的 `strings` 扫描未发现 `/Users/liuqingxing` 或项目工作区绝对路径；`hdiutil verify src-tauri/target/release/bundle/dmg/稿匣_0.1.3_aarch64.dmg` 通过；`.app` 内置 sidecar 本地 Embedding 检查通过；`.app` 主程序启动并保持运行 10 秒通过；`npm run package:test:macos` 通过，生成 `release/test-release/macos/稿匣_0.1.3_测试包`，在该目录执行 `shasum -a 256 -c SHA256SUMS.txt` 通过，返回 `稿匣_0.1.3_aarch64.dmg: OK`。GitHub Actions `Windows Desktop Release` run `27259635805` 在分支 `codex/full-verification-windows-20260609` 的提交 `943cd574b2ffa7fc9f8e486377f31b6d400c5d14` 通过，产物包含 `稿匣_0.1.3_x64-setup.exe` 和 `novel-backend-x86_64-pc-windows-msvc.exe`；本地下载 artifact `gaoxia-windows-943cd574b2ffa7fc9f8e486377f31b6d400c5d14` 后整理到 `release/test-release/windows/稿匣_0.1.3_测试包`，在该目录执行 `shasum -a 256 -c SHA256SUMS.txt` 通过，两个可执行文件均返回 `OK`。Windows 实机安装、卸载、首次启动和安装后 GUI 操作仍未验证。

## 2026-06-09

### 发布就绪聚合审计脚本

- 修改摘要：新增 `npm run verify:release-audit`，按顺序执行 `npm run verify`、`npm run verify:local-smoke`、`.venv/bin/python scripts/verify-weicheng-original-continuation.py --local-only` 和 `npm run verify:model-preflight`，并输出 JSON 汇总；任何必需检查失败都会返回非零退出码，避免在真实模型或关键本地回归未通过时误判为可上线。
- 影响范围：发布验证命令、打包静态检查、README、技能流程回归清单和项目 Agent 指令；不改变产品运行时接口、模型配置结构、章节生成逻辑、旧稿接管流程或桌面打包产物。
- 验证结果：`node --check scripts/verify-release-audit.mjs scripts/verify-packaging-static.mjs scripts/verify-frontend-static.mjs` 通过；`npm run verify:packaging-static` 通过；2026-06-09 12:08 UTC 执行 `npm run verify:release-audit` 返回非零退出码，汇总显示 `npm run verify` 通过（包含前端静态回归、API 冒烟、431 个后端 unittest 和前端生产构建），`npm run verify:local-smoke` 通过，`.venv/bin/python scripts/verify-weicheng-original-continuation.py --local-only` 通过，读取默认《围城》原文 212352 字并写入 9 章、接续章号为第 10 章、知识检索命中 8 条、第 10 章上下文长度 5610；`npm run verify:model-preflight` 未通过，写作模型 `qwen3-max` 和第二审查模型 `qwen/qwen3.7-max` 均指向 `https://api.qnaigc.com/v1/chat/completions`，URL 格式合法且 API Key 存在，但本机 DNS 解析 `api.qnaigc.com` 返回 `nodename nor servname provided, or not known`，`blockedBy` 为 `model-preflight`。当前仍不能判定真实模型长篇生成已通过。

### 模型配置预检脚本

- 修改摘要：新增 `npm run verify:model-preflight`，读取当前保存的 `app_config.json`，检查写作模型和已启用第二审查模型的模型名、接口地址、API Key 是否存在，以及接口域名 DNS 是否可解析；脚本不输出 API Key、不发送模型请求，用于在真实模型验证前区分配置 / DNS 问题和生成质量问题。
- 影响范围：发布验证命令、打包静态检查、README、技能流程回归清单和项目 Agent 指令；不改变模型配置结构、模型调用传输层、设置页表单、真实模型验证脚本或章节生成逻辑。
- 验证结果：`.venv/bin/python -m py_compile scripts/verify-model-preflight.py scripts/verify-api-release-smoke.py scripts/verify-local-release-smoke.py` 通过；`npm run verify:packaging-static` 通过；2026-06-09 执行 `npm run verify:model-preflight` 未通过，结果显示写作模型 `qwen3-max` 和第二审查模型 `qwen/qwen3.7-max` 的 URL 格式合法、API Key 存在，均指向 `https://api.qnaigc.com/v1/chat/completions`，但本机 DNS 解析 `api.qnaigc.com` 返回 `nodename nor servname provided, or not known`，因此真实模型脚本仍不能进入生成阶段；随后执行 `npm run verify` 通过，包含打包静态检查、前端静态回归、API 冒烟、431 个后端 unittest 和前端生产构建，后端测试用时 85.946 秒。

### API 发布冒烟验证脚本

- 修改摘要：新增 `npm run verify:api-smoke`，使用 FastAPI 测试客户端在临时数据目录中直接验证主要 API 路由，不启动本地端口，也不调用写作模型。脚本覆盖配置保存、本地 Embedding 配置测试、旧稿接管、作品列表和重命名、架构文档、项目记忆、Agent 线程、资料导入和检索、章节保存后的项目记忆核验、快照、自动保存、整书导出、迁移包导出和迁移包导入；`npm run verify` 已加入这项检查。
- 影响范围：发布验证命令、打包静态检查、README、技能流程回归清单和项目 Agent 指令；不改变运行时 API、模型配置结构、章节保存逻辑、真实模型验证脚本或浏览器 UI smoke。
- 验证结果：2026-06-09 11:41 UTC 执行 `npm run verify:api-smoke` 通过，本地 Embedding 返回 `BAAI/bge-small-zh-v1.5 可用，向量维度 512`，旧稿接管写入 3 章、接续章号为第 4 章，知识检索 6 条命中，章节检索 5 条命中，章节核验 77/100（需关注），共 9 个问题，其中项目记忆规则维度 2 个 critical，快照、整书导出、迁移包导出和迁移包导入均通过；随后执行 `npm run verify` 通过，包含打包静态检查、前端静态回归、API 冒烟、431 个后端 unittest 和前端生产构建，后端测试用时 96.175 秒。2026-06-09 对当前模型配置做非敏感审计，写作模型和第二审查模型均指向 `https://api.qnaigc.com/v1`，URL 格式合法且 API Key 存在，但本机 DNS 解析 `api.qnaigc.com` 返回 `ENOTFOUND`，因此真实模型脚本仍无法进入生成阶段。

### 本地发布冒烟验证脚本

- 修改摘要：新增 `npm run verify:local-smoke`，通过临时数据目录直接调用后端服务，验证本地 Embedding、旧稿接管、接续章上下文、资料导入 / 检索、章节写入后的知识索引刷新，以及本地章节核验。脚本会在临时项目里人为写入连续性错误，清空常见模型 API Key 环境变量，并跳过远端章节审查模型和叙事编辑模型，避免在当前环境误发起外部模型调用。
- 影响范围：发布验证命令、打包静态检查、README、技能流程回归清单和项目 Agent 指令；不改变产品运行时接口、模型配置、旧稿导入数据结构、章节保存逻辑或真实模型验证脚本。
- 验证结果：`.venv/bin/python -m py_compile scripts/verify-local-release-smoke.py` 通过；`npm run verify:packaging-static` 通过；2026-06-09 11:39 UTC 执行 `npm run verify:local-smoke` 通过，本地 Embedding 返回 `BAAI/bge-small-zh-v1.5 可用，向量维度 512`，旧稿接管写入 3 章、接续章号为第 4 章，知识检索和章节检索各返回 3 条命中，本地章节核验 77/100（需关注）并识别 2 个 critical 项目记忆违规；2026-06-09 执行 `npm run verify` 通过，包含打包静态检查、前端静态回归、API 冒烟、431 个后端 unittest 和前端生产构建。该脚本不替代 `npm run verify:ui`、`npm run verify:desktop` 或真实模型长篇验证。

### Agent 运行状态与前端静态回归检查

- 修改摘要：新增 `npm run verify:frontend-static`，不启动本地端口，检查旧稿接管界面不再展示拆章置信度、导入报告展示“已导入章节”、Agent 运行中保留状态列表，且完成后的历史消息不展示执行时间线或事件阶段摘要。Agent 历史模板显式排除完成态消息的时间线和事件摘要，后端 workflow 摘要测试新增运行状态字段断言，覆盖“已完成 / 正在运行 / 正在思考”列表所需的动作、消息和子任务状态。
- 影响范围：前端验证命令、`NovelWorkflowPanel` 历史消息模板、Agent workflow 摘要测试、README、技能流程回归清单和打包静态检查；不改变后端接口、章节数据结构、真实模型调用或旧稿接管导入流程。
- 验证结果：`node --check scripts/verify-frontend-static.mjs` 通过；`npm run verify:frontend-static` 通过；`npm run verify:packaging-static` 通过；`.venv/bin/python -m unittest backend.tests.test_agent_workflow_service -v` 通过，6 个用例通过；2026-06-09 执行 `npm run verify` 通过，包含打包静态检查、前端静态回归、431 个后端 unittest 和前端生产构建。2026-06-09 再次执行 `npm run verify:ui` 仍被当前 Codex 沙箱的本地监听权限阻止，错误为 `listen EPERM: operation not permitted 127.0.0.1`；随后尝试使用 Browser 插件打开 `http://localhost:1420/`，插件安全策略拒绝该地址，未进入页面点击验证。用户在内置浏览器打开 `http://localhost:1420/` 并明确授权后，Browser 插件仍拒绝读取该页 DOM；用户在 Google Chrome 打开页面后，Chrome 插件本次没有发现 `localhost:1420` 标签，尝试新开同一地址时仍被同一条浏览器安全策略拒绝，页面点击验证仍未执行。

### 旧稿接管准确性表达调整

- 修改摘要：旧稿接管成功提示和导入弹窗不再显示规则拆章置信度，报告区域改成展示“已导入章节”；导入前说明只保留旧稿正文导入、写入章节和刷新本地知识库索引，避免把规则拆章表达成模型理解或准确性保证。`scripts/verify-weicheng-original-continuation.py` 新增 `--local-only` 模式，并把真实模型配置测试移到原文拆章、导入、知识索引和第 10 章上下文校验之后；真实模型不可达时，脚本输出仍保留本地导入和接续上下文证据。
- 影响范围：旧稿接管前端弹窗、导入成功提示、真实原文续写验证脚本、README、技能流程回归清单和项目 Agent 指令；不改变旧稿导入接口、报告数据结构、章节写入方式、知识库刷新或正式章节生成 API。
- 验证结果：`.venv/bin/python -m py_compile scripts/verify-weicheng-original-continuation.py` 通过；`npm run build` 通过；2026-06-09 执行 `npm run verify` 通过，包含打包静态检查、前端静态回归、431 个后端 unittest 和前端生产构建；同日再次执行 `npm run verify:ui`，被当前 Codex 沙箱的本地监听权限阻止，错误为 `listen EPERM: operation not permitted 127.0.0.1`，脚本未进入页面断言。2026-06-09 12:01 UTC 执行 `.venv/bin/python scripts/verify-weicheng-original-continuation.py --local-only` 通过，读取默认《围城》原文 212352 字，拆成 9 章并写入 9 章，接续章号为第 10 章，知识检索命中 8 条，方鸿渐 / 鸿渐命中 4 条，孙柔嘉 / 柔嘉命中 4 条，第 10 章上下文长度 5610；2026-06-09 11:59 UTC 执行 `.venv/bin/python scripts/verify-weicheng-original-continuation.py --allow-real-model-calls --target-words 900` 时，原文拆章、导入、知识索引和第 10 章上下文先通过，随后写作模型和第二审查模型仍因 DNS 解析失败退出，本地 Embedding 通过，未进入真实续写生成。

### 模型 DNS 解析失败重试增强

- 修改摘要：模型错误分类和统一 JSON 传输层把 `[Errno 8] nodename nor servname provided, or not known`、`name or service not known`、`temporary failure in name resolution` 等 DNS 解析失败归为可重试的模型网络连接中断；聊天模型请求、设置页模型测试和真实长篇验证脚本会按 `NOVEL_MODEL_RETRY_DELAYS` 等待间隔重试，不再在第一次 DNS 抖动时直接失败。
- 影响范围：模型请求错误分类、聊天模型 JSON 传输、设置页“测试当前配置”、真实模型验证脚本的配置测试阶段、README、核心引擎说明、桌面版方案和项目 Agent 指令；不改变模型配置结构、API Key 保存方式、Embedding 本地模型或章节数据格式。
- 验证结果：`.venv/bin/python -m unittest backend.tests.test_model_error_service backend.tests.test_model_transport_service -v` 通过，18 个用例通过；再次执行 `.venv/bin/python scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型仍因当前网络 DNS 解析失败未进入章节生成，但错误已被归类为“模型网络连接中断”，并且两者均显示 `已尝试 6 次`；2026-06-09 执行 `.venv/bin/python scripts/verify-weicheng-original-continuation.py --allow-real-model-calls --target-words 900`，读取 `/Users/liuqingxing/Desktop/围城续写/.claude/skills/weicheng/resources/original-text.md` 后在配置测试阶段失败，本地 Embedding 通过，写作模型和第二审查模型均因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入原文导入和续写生成。
- 补充验证：2026-06-09 10:17 UTC 执行 `.venv/bin/python scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型均因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入章节生成；2026-06-09 10:18 UTC 再次执行 `.venv/bin/python scripts/verify-weicheng-original-continuation.py --allow-real-model-calls --target-words 900`，同样停在模型配置测试阶段，本地 Embedding 通过，写作模型和第二审查模型均因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入原文导入或续写。
- 补充验证：2026-06-09 10:35 UTC 再次执行 `.venv/bin/python scripts/verify-weicheng-original-continuation.py --allow-real-model-calls --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型均因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入原文导入或续写；2026-06-09 10:36 UTC 再次执行 `.venv/bin/python scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型同样因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入章节生成。
- 补充验证：2026-06-09 11:12 UTC 再次执行 `.venv/bin/python scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型仍因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入章节生成。
- 补充验证：2026-06-09 11:23 UTC 执行 `.venv/bin/python scripts/verify-weicheng-original-continuation.py --allow-real-model-calls --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型均因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入原文导入或续写；2026-06-09 11:24 UTC 执行 `.venv/bin/python scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型同样因 DNS 解析失败退出并显示 `已尝试 6 次`，未进入章节生成。

### 单元测试模型调用隔离

- 修改摘要：Agent、Studio 和自学习相关单测在写入章节或刷新叙事状态时，不再触发真实叙事编辑模型或章节审查模型；这些测试改用明确的测试替身，避免外网 DNS 状态影响本地回归。
- 影响范围：Agent 服务测试、Studio 服务测试、自学习服务测试和后端完整 unittest；不改变运行时章节保存、章节核验、叙事状态维护或真实模型调用链路。
- 验证结果：完整后端测试探针通过，430 个用例通过且 `NETWORK_CALL_TOTAL 0`；`.venv/bin/python -m unittest backend.tests.test_agent_service backend.tests.test_studio_service backend.tests.test_self_evolution_service -v` 通过，80 个用例通过；`npm run verify` 通过，包含打包静态检查、430 个后端 unittest 和前端生产构建，后端测试用时 86.929 秒；2026-06-09 再次执行 `npm run verify` 通过，431 个后端 unittest 用时 100.511 秒，前端生产构建通过。

### macOS release 打包验证加固

- 修改摘要：macOS 桌面发布验证默认改用 Tauri release 产物，不再把 debug DMG 作为对外测试包来源；仍可通过 `TAURI_BUILD_PROFILE=debug` 临时验证 debug 包。Tauri 构建会加入 Rust 路径重映射，release `.app` 主程序和 sidecar 会扫描开发机用户目录与工作区绝对路径，避免调试路径进入第三方分发包。macOS 和 Windows 的 PyInstaller sidecar 打包都把 `PYINSTALLER_CONFIG_DIR` 指向临时构建目录，避免依赖用户目录缓存可写。macOS 测试包信息新增构建类型。
- 影响范围：`verify-desktop-release.sh`、`prepare-macos-test-release.sh`、`repair-tauri-app-signature.sh`、macOS / Windows sidecar 打包脚本、打包静态检查、README、桌面发布回归说明、桌面版方案和技能流程回归清单；不改变应用运行接口、项目数据格式、许可证格式、模型配置或前端功能。
- 验证结果：`bash -n scripts/build-backend-sidecar.sh scripts/verify-desktop-release.sh scripts/prepare-macos-test-release.sh scripts/repair-tauri-app-signature.sh` 通过；`node --check scripts/verify-packaging-static.mjs` 通过；`npm run verify:packaging-static` 通过；`npm run backend:bundle` 通过，PyInstaller 日志显示缓存清理路径为 `/tmp/novel-sidecar-build.../config/pyinstaller`；`npm run verify:desktop` 在当前 Codex 沙箱中执行到 sidecar 健康检查前，已通过 428 个后端 unittest、前端生产构建和 sidecar 打包，随后因沙箱禁止本地监听 socket 失败，错误为 `PermissionError: [Errno 1] Operation not permitted`，不能作为完整桌面验证通过证据；带 `RUSTFLAGS="--remap-path-prefix=/Users/liuqingxing/Desktop/小说生成器=/workspace --remap-path-prefix=/Users/liuqingxing=/home"` 执行 `npm run tauri -- build` 时，release `.app` 已生成，但 DMG 阶段 `bundle_dmg.sh` 失败，当前沙箱无法执行脚本所需的 `hdiutil attach`，不能作为 DMG 生成通过证据；对 release `.app` 执行 `bash scripts/repair-tauri-app-signature.sh '稿匣' release` 后，`codesign --verify --deep --strict --verbose=2 src-tauri/target/release/bundle/macos/稿匣.app` 通过；`strings` 扫描 release `.app/Contents/MacOS/novel-generator` 和 `novel-backend` 未发现 `/Users/liuqingxing`、本项目绝对路径、`.cargo`、`.rustup` 或 `Library` 开发机路径。
- 补充验证：2026-06-09 再次执行 `npm run verify` 通过，包含打包静态检查、428 个后端 unittest 和前端生产构建；再次执行 `npm run verify:ui` 被当前 Codex 沙箱的本地监听权限阻止，错误为 `listen EPERM: operation not permitted 127.0.0.1`，不能作为 UI smoke 通过证据。
- 补充验证：2026-06-09 再次执行 `npm run verify:ui`，仍被当前 Codex 沙箱的本地监听权限阻止，错误为 `listen EPERM: operation not permitted 127.0.0.1`，脚本未进入界面断言；同日再次执行 `npm run verify:desktop`，打包静态检查、430 个后端 unittest、前端生产构建和 Python sidecar 打包均已完成，随后在打包后 sidecar 健康检查阶段被沙箱阻止，错误为 `PermissionError: [Errno 1] Operation not permitted`，不能作为完整桌面验证通过证据。
- 补充验证：2026-06-09 18:37 Asia/Shanghai 再次执行 `npm run verify:desktop`，打包静态检查通过，430 个后端 unittest 通过，前端生产构建通过，Python sidecar 打包成功并生成 `src-tauri/binaries/novel-backend-aarch64-apple-darwin`；随后在打包后 sidecar 健康检查阶段被当前 Codex 沙箱阻止，错误为 `PermissionError: [Errno 1] Operation not permitted`，未进入 Tauri `.app/.dmg` 构建和应用启动检查。

### 原文接管续写真实模型验证

- 修改摘要：新增 `scripts/verify-weicheng-original-continuation.py`，用于读取指定原文文件，走旧稿接管拆章、项目创建、知识索引、第 10 章上下文构建、真实写作模型续写、章节保存、第二审查模型核验、自动修订检查和保存后知识检索。脚本默认拒绝真实模型调用，必须传 `--allow-real-model-calls`；续写正文不会打印到终端，只输出章节数、命中数、核验状态和混淆检查结果。同步收紧 `scripts/verify-real-model-longform.py` 的真实长篇质量门，连续性合同、Obsidian 必写 / 禁写、critical 和必须修订问题在自动修订后仍存在时会直接失败；自动修订提示增加连续性合同项的可核验关键词要求；UI smoke 的 Obsidian 同步等待时间改为 60 秒，降低本地同步偶发慢响应导致的误报。
- 影响范围：真实模型验证脚本、章节自动修订提示、UI smoke 等待策略、README、技能流程回归清单和项目 Agent 指令；不改变旧稿接管接口、作品文件格式、模型配置结构、前端设置项或正式章节生成 API。
- 验证结果：`.venv/bin/python -m py_compile scripts/verify-weicheng-original-continuation.py scripts/verify-real-model-longform.py backend/novel_backend/services/chapter_auto_repair_service.py` 通过；不带 `--allow-real-model-calls` 执行 `scripts/verify-weicheng-original-continuation.py` 会拒绝调用真实模型并返回退出码 2；`.venv/bin/python scripts/verify-weicheng-original-continuation.py --allow-real-model-calls --target-words 900` 通过，真实模型配置测试返回写作模型 `qwen3-max` passed、耗时 2.957 秒，本地 Embedding `BAAI/bge-small-zh-v1.5` passed、512 维、耗时 0.536 秒，第二审查模型 `qwen/qwen3.7-max` passed、耗时 3.949 秒；原文拆成 9 章并全部导入，接续章号为第 10 章，知识检索和第 10 章上下文均覆盖方鸿渐 / 鸿渐与孙柔嘉 / 柔嘉；第 10 章真实续写生成 1084 字符，用时 334.331 秒，保存和核验用时 136.554 秒，核验 81/100（需关注），critical、连续性合同、必须修订、Obsidian 必写和 Obsidian 禁写问题均为 0，自动修订未触发；混淆检查未发现林追、铜钥匙、白石商会、灰港等其它测试项目元素，也未发现方鸿渐婚姻关系写错；保存后用新章唯一片段检索命中第 10 章。`node --check scripts/verify-ui-smoke.mjs` 通过；`npm run verify:ui` 通过；`.venv/bin/python -m unittest backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_default_second_round_when_first_round_still_has_required_issue backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_chapter_scoped_obsidian_required_phrase backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_project_memory_state_rule_issues backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_project_memory_reveal_rule_issues -v` 通过，4 个用例通过；`npm run verify` 通过，包含打包配置静态检查、428 个后端 unittest 和前端生产构建；`npm run verify:desktop` 通过，包含打包配置静态检查、428 个后端 unittest、前端生产构建、Python sidecar 打包、独立 sidecar 健康检查、本地 Embedding 检查、Tauri debug `.app` / `.dmg` 构建、签名修复、应用内 sidecar 健康检查和 `.app` 启动检查；`npm run package:test:macos` 通过，重新生成 `release/test-release/macos/稿匣_0.1.2_测试包`；在测试包目录执行 `shasum -a 256 -c SHA256SUMS.txt` 通过，返回 `稿匣_0.1.2_aarch64.dmg: OK`；`hdiutil verify release/test-release/macos/稿匣_0.1.2_测试包/稿匣_0.1.2_aarch64.dmg` 通过；`codesign -dv --verbose=4` 确认 `.app` 主程序和 `novel-backend` sidecar 均为 ad-hoc 签名且 `TeamIdentifier=not set`，测试包 `包信息.txt` 和安装说明已标明未 notarize；对测试包和 `.app` 扫描 `PRIVATE KEY / private_key / .gaoxia-license / 常见 API Key 环境变量 / sk-...` 没有命中；`.app/Contents` 只包含 `Info.plist`、`novel-generator`、`novel-backend`、图标和 `_CodeSignature`，未发现 `app_config.json`、`license.json`、`knowledge.db`、日志、项目目录或 `.gaoxia` 数据；当前 Codex 沙箱无法挂载 DMG，`hdiutil attach` 返回 `Cannot start hdiejectd because app is sandboxed`，因此 DMG 内部目录未能在本机直接列出。真实多章验证中，`scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900` 曾暴露第 2 章自动修订后仍残留 4 个连续性合同类必须修订问题；后续复验曾遇到一次 `SSL UNEXPECTED_EOF_WHILE_READING`，不能作为通过证据。
- 补充验证：2026-06-09 09:19 UTC 再次执行 `.venv/bin/python scripts/verify-real-model-longform.py --allow-real-model-calls --chapters 2 --target-words 900`，许可证有效，本地 Embedding 通过，写作模型和第二审查模型均因 DNS 解析失败退出，错误为 `[Errno 8] nodename nor servname provided, or not known`，未进入章节生成，不能作为真实多章长篇通过证据。

### 真实模型长篇验证脚本

- 修改摘要：新增 `scripts/verify-real-model-longform.py`，用于复制当前本机保存的 `app_config.json` 和许可证到临时数据目录，创建临时长篇作品后真实调用写作模型、本地 Embedding 和第二审查模型，验证章节生成、保存、章节核验、项目记忆硬规则和知识检索。脚本默认拒绝外部模型调用，必须传 `--allow-real-model-calls`；默认生成 1 章，支持 `--chapters 2` / `--chapters 3` 扩展，也支持 `--skip-auto-repair` 跳过保存后自动修订以单独观察生成、保存、核验和检索耗时。脚本运行时会输出模型测试、章节生成、保存核验、自动修订检查和知识检索阶段，避免真实调用长时间等待时不知道停在哪一步。
- 影响范围：真实模型验证脚本、README 和技能流程回归清单；不改变产品主流程、后端 API、模型配置结构、章节生成提示词或前端界面。
- 验证结果：`.venv/bin/python -m py_compile scripts/verify-real-model-longform.py` 通过；不带 `--allow-real-model-calls` 执行脚本会拒绝调用真实模型并返回退出码 2；带 `--allow-real-model-calls --chapters 1 --target-words 900` 执行通过，真实模型配置测试返回写作模型 `qwen3-max` passed、耗时 2.136 秒，本地 Embedding `BAAI/bge-small-zh-v1.5` passed、512 维、耗时 0.501 秒，第二审查模型 `qwen/qwen3.7-max` passed、耗时 7.724 秒；临时项目知识索引 11 个 chunk；第 1 章真实生成 847 字符，用时 222.24 秒，保存和章节核验用时 166.734 秒，核验 85/100（良好），自动修订未触发；知识检索命中 6 条，包含章节正文和铜钥匙相关资料。另一次 `--chapters 2 --target-words 900` 运行中，第 1 章通过，生成 847 字符、核验 84/100（需关注）；第 2 章进入自动修订后的叙事状态模型调用时等待过久，被人工中断，不能作为两章通过证据。
...
- 修改摘要：真实调用当前保存配置执行临时四章项目验证，确认写作模型 `qwen3-max`、打包内本地知识检索模型 `BAAI/bge-small-zh-v1.5` 和第二审查模型 `qwen/qwen3.7-max` 均可用；完整章生成链路曾在 SSL EOF 下失败，随后把聊天模型请求和设置页模型测试的可重试网络错误默认重试次数增至 5 次，并支持 `NOVEL_MODEL_RETRY_DELAYS` 调整等待间隔。章节自动修订默认次数从 1 轮改为 2 轮，已启用但仍保存为旧 1 轮的配置会在读取时升级为 2 轮，并新增多轮修订回归测试。
- 影响范围：写作模型调用、第二审查模型调用、模型设置测试、章节自动修订默认配置、旧配置迁移、连续性合同问题修订、README、核心引擎说明、桌面版方案、项目 Agent 指令和相关单测；不改变模型名、接口地址、API Key 配置、本地 Embedding、项目文件格式或章节核验维度结构。
- 验证结果：真实模型配置测试 passed：写作模型 `qwen3-max` 3.758 秒，本地 Embedding 512 维 0.527 秒，第二审查模型 `qwen/qwen3.7-max` 2.182 秒；真实临时项目完整生成第 1 章 1363 字符，保存后核验 86/100（良好），`项目记忆规则` 94/100 且无违规；真实生成第 2 章 1585 字符，自动修订后保存 1870 字符，核验 80/100（需关注），`项目记忆规则` 94/100 且无违规，脚本限制 1 轮自动修订时仍残留 4 个连续性合同类必须修订问题；追加真实两章验证中，本地 Embedding 知识索引 5 个 chunk 成功，第一章核验 78/100（需关注），第二章核验 72/100（需关注）并触发 1 轮真实 `chapter_auto_repair`，修订调用耗时 41.838 秒；执行真实脚本时发现本机旧配置仍是 1 轮，修复后读取真实配置确认 `auto_repair_max_rounds=2`。`.venv/bin/python -m unittest backend.tests.test_config_service backend.tests.test_model_transport_service backend.tests.test_model_error_service backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_chapter_scoped_obsidian_required_phrase backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_default_second_round_when_first_round_still_has_required_issue backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_project_memory_state_rule_issues backend.tests.test_project_service.ProjectServiceTestCase.test_auto_repair_uses_project_memory_reveal_rule_issues -v` 通过，33 个用例通过；`.venv/bin/python -m unittest backend.tests.test_config_service -v` 通过，14 个用例通过；`npm run backend:test` 通过，428 个后端 unittest 通过；`npm run verify` 通过，包含打包配置静态检查、428 个后端 unittest 和前端生产构建；`npm run verify:desktop` 通过，包含后端 unittest、前端构建、Python sidecar 打包、内置本地 Embedding 健康检查、Tauri debug `.app` / `.dmg` 构建、签名检查、应用内 sidecar 健康检查和 `.app` 启动检查。
...
- 影响范围：写作模型调用、第二审查模型调用、模型设置测试、联网模型传输层、README、核心引擎说明、项目 Agent 指令和相关单测；不改变模型名、接口地址、API Key 配置、本地 Embedding、提示词内容、章节审校规则或失败后的错误提示。
- 验证结果：`.venv/bin/python -m unittest backend.tests.test_model_transport_service backend.tests.test_model_error_service -v` 通过，12 个用例通过，覆盖聊天模型请求和设置页模型测试在连续 3 次 SSL EOF 后第 4 次成功返回；`npm run backend:test` 通过，422 个后端 unittest 通过；`npm run verify` 通过，包含打包配置静态检查、422 个后端 unittest 和前端生产构建；使用当前保存配置执行真实模型配置测试通过，写作模型 `qwen3-max`、知识检索模型 `BAAI/bge-small-zh-v1.5` 和第二审查模型 `qwen/qwen3.7-max` 均为 passed。
